...
namespace BigIntegerTests
{
    public class Methods
    {
        ...
        public void TestGetBytes()
        {
            { // Randomized tests
                ...
                for (int j = 0; j < 512; j++)
                {
                    ...
                    Assert.Equal(bytes_got_back.Length, byte_array.Length);

                    for (var i = 0; i < byte_array.Length; i++)
                        Assert.Equal(bytes_got_back[i], byte_array[i]);
                }
            }
            ...
            { // Special case - zero
                ...
                Assert.NotEmpty(zero_bytes);
                Assert.Equal(zero_bytes[0], (byte)0);
            }
        }
        ...
        public void TestModPow()
        {
            ...
            Assert.Equal(868041175, res.LongValue());
            ...
            Assert.Equal(new BigInteger("676144631297564803799040568236788209319025642240115630978591468748134664779002", 10), res);
        }
        ...
        public void TestModInverse()
        {
            ...
            Assert.Equal(736445995, b.LongValue());

            b = a.modInverse(new BigInteger(1999));
            Assert.Equal(1814, b.LongValue());
        ...
        public void TestJacobi()
        {
            ...
            for (int i = 0; i < jcbTbl.Length / 3; i++)
            {
                ...
                Assert.Equal(jcbTbl[i, 2], BigInteger.Jacobi(a, b));
            }
        ...
        public void TestSecuredGenRandomBits()
        {
            ...
            { // Test upper boundary values
                ...
                Assert.Equal(70, bi.dataLength);

                bi.genRandomBits(2239, rng);
                Assert.Equal(70, bi.dataLength);
                Assert.Equal(2239, bi.bitCount());
            }
            ...
            { // Test lower boudary value
                ...
                Assert.True(bi.getBytes()[0] == 1 || bi.getBytes()[0] == 0);
            }
        }
        ...
        public void TestAbs()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.Equal(Math.Abs(val), bi.abs());
            }
            ...
            Assert.Equal("4809238490238509385094809584086094850909458309580485093485093485094809580945809458340324342343242342343242", bi.abs().ToString());

            bi = new BigInteger();
            Assert.Equal(0, bi.abs());
            ...
            Assert.Equal("38265236482749823794237948792386482364236462846234623862368236423764236624762384762384623862376482364823", bi.abs().ToString());
        }
        ...
        public void TestMax()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.Equal(Math.Max(val1, val2), bi1.max(bi2));
                Assert.Equal(bi1.max(bi2), bi2.max(bi1));
                Assert.Equal(bi1, bi1.max(bi1));
            }
            ...
            Assert.Equal(bi1, bi1.max(bi2));
            Assert.Equal(bi1, bi2.max(bi1));
        ...
        public void TestMin()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.Equal(Math.Min(val1, val2), bi1.min(bi2));
                Assert.Equal(bi1.min(bi2), bi2.min(bi1));
                Assert.Equal(bi1, bi1.min(bi1));
            }

            bi1 = new BigInteger("49823798573298479823749823798472398479238479823749823749823794837298472398469238649836294862398462398649823649823649823649823694", 10);
            bi2 = new BigInteger("-37209847385984792370497230948093284092384092380958058094809238409328049238094237094723094723984792384792387492379487239847239847923847923879847923847239847", 10);
            Assert.Equal(bi2, bi1.min(bi2));
            Assert.Equal(bi2, bi2.min(bi1));
            ...
            bi2 = new BigInteger("49823798573298479823749823798472408479238479823749823749823794837298472398469238649836294862398462398649823649823649823649823694", 10);
            Assert.Equal(bi1, bi1.min(bi2));

            bi1 = new BigInteger("-9852375989470234802398402398409238049238094723094709234702387498237498623948623984623984623864237642376482376492386479238749823749237498237498237", 10);
            Assert.Equal(bi1, bi1.min(0));

            bi2 = new BigInteger("287498237498623846236236826386276327638276327632763276382763872688947329847923847982374982379482379847239847392847982374982374982374982374982374982379482379482379487239847239847", 10);
            Assert.Equal(bi1, bi2.min(bi1));
        ...
        public void TestSqrt()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.Equal(sqrtVal, bi.sqrt());
            }

            bi = new BigInteger();
            Assert.Equal(0, bi.sqrt());

            bi = new BigInteger("48234798239584935745984795837", 10);
            Assert.Equal(219624220521291, bi.sqrt());
            ...
            Assert.Equal("69451276618637425696010359184467375646677653070095660334837", bi.sqrt().ToString());
            ...
            Assert.Equal("949937153042746085485800690340716910200218535446376464883006159759187016711766033117259286191698487700345112712284215083646265481183724", bi.sqrt().ToString());
        }
        ...
        public void TestGCD()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.Equal(GCD(val1, val2), bi1.gcd(bi2));
                Assert.Equal(bi1.gcd(bi2), bi2.gcd(bi1));
            }
            ...
            Assert.Equal(12604189422825, bi2.gcd(bi1));
            Assert.Equal(bi1.gcd(bi2), bi2.gcd(bi1));
        ...
        public void TestToHexString()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                Assert.Equal(valHexString, bi.ToHexString());
            }
            ...
            Assert.Equal("86042e915cdcf19902845ddf57c6b604685c53a01da858573f52219e1c743fc193e5b56aaba29ef308a82cd26da8066d1ae2af170b1443f3b539938966107f8f77263e4f13fb815049d5f746438519".ToUpper(),
                bi.ToHexString());
        }
    }
}
